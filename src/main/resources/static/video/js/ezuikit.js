...
(function (global, factory) {
  ...
  // 播放相关API
  EZUIKitPlayer.prototype.play = function (url) {
    var id = 'EZUIKitPlayer-' + this.opt.id;
    var player = document.getElementById(id).contentWindow;
    player.postMessage({action:"play",accessToken: this.opt.accessToken,url:url}, domain + "/ezopen/h5/iframe")
  }
  EZUIKitPlayer.prototype.stop = function () {
    var id = 'EZUIKitPlayer-' + this.opt.id;
    var player = document.getElementById(id).contentWindow;
    return new Promise((resolve, reject)=>{
      resolve(player.postMessage("stop", domain + "/ezopen/h5/iframe"));
    });
  }
